...
const sliderBaner = new Swiper('.baner__swiper', {
    ...
    navigation: {
        nextEl: '.baner__swiper-btn-next',
        prevEl: '.baner__swiper-btn-prev'
    },
    on: {
        init() {
...
function getElems() {
    return (
        (banerTitle = $('.baner__slide-title')),
        (banerText = $('.baner__slide-text')),
        (banerNumber = $('.baner__slide-number')),
        (banerImg = $('.baner__img')),
        (banerMiniImg = $('.baner__slide-mini-img')),
        (banerMiniImgBg = $('.baner__slide-mini-img-bg')),
        (banerLink = $('.baner__slide-link')),
        (banerLinkTitle = $('.baner__slide-link-title')),
        (banerLinkSub = $('.baner__slide-link-sub')),
        (banerLinkArrow = $('.baner__slide-link-arrow')),
        (banerLinkBg = $('.baner__slide-link-bg'))
    );
}
function lockBtns() {
    $('.baner__swiper-btn-prev').attr('disabled', 'disabled');
    $('.baner__swiper-btn-next').attr('disabled', 'disabled');
}
function unlockBtns() {
    $('.baner__swiper-btn-prev').removeAttr('disabled', 'disabled');
    $('.baner__swiper-btn-next').removeAttr('disabled', 'disabled');
}
function onInitShow(index) {
    getElems();
    ...
    timeline2
        ...
        .to(
            [
                ...
                banerNumber[index],
                banerMiniImg[index],
                banerLinkTitle[index],
                banerLinkSub[index],
                banerLinkArrow[index]
            ],
            {
                opacity: 1,
                duration: 1.6,
                onComplete: unlockBtns
            }
        );
}
function prevSlideAnim(index) {
    getElems();
...
function activeSlideAnim(index) {
    getElems();
    $('.swiper-slide-next').css('opacity', 1);
...
$('.competence-item').each(function () {
    let timelineCompetence = gsap.timeline({
        scrollTrigger: {
            trigger: this,
            start: 'top 70%'
            // markers: true,
...
const pinsElems = $('.world-container-map-info-item');
...
pinsElems.on('click', function () {
    sliderFactory.slideTo(+$(this).attr('data-index'));
});

function refreshPins(index = 0) {
    pinsElems.removeClass('active');

    $(pinsElems[index]).addClass('active');
}
